  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1">Find and reconnect with your classmates</p>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            ...
            <p className="text-sm text-gray-500 mt-1">Total Alumni</p>
          </div>
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <p className="text-3xl font-bold text-indigo-600">{stats.cityDistribution.length}</p>
            <p className="text-sm text-gray-500 mt-1">Cities</p>
          </div>
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <p className="text-3xl font-bold text-indigo-600">{stats.yearDistribution.length}</p>
            <p className="text-sm text-gray-500 mt-1">Graduation Years</p>
          </div>
          <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
            <p className="text-3xl font-bold text-indigo-600">{stats.cityDistribution[0]?.city || '-'}</p>
            <p className="text-sm text-gray-500 mt-1">Top City</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wide">Graduation Year</label>
            <input
              type="number"
        ...
        {hasActiveFilters && (
          <div className="mt-3 flex items-center justify-between">
            <p className="text-sm text-gray-500">
              Found <span className="font-semibold text-gray-800">{total}</span> result{total !== 1 ? 's' : ''}
            </p>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <button onClick={fetchAlumni} className="mt-4 text-indigo-600 hover:underline">Try again</button>
        </div>
      ) : alumni.length === 0 ? (
        <div className="text-center py-20">
          ...
          <p className="text-gray-500 text-sm mt-1">
            {hasActiveFilters ? 'Try adjusting your filters' : 'Be the first to add your profile!'}
          </p>
        </div>
      ) : (
        <>
