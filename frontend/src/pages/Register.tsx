...
const Register: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.username || !form.email || !form.password || !form.confirmPassword) { setError(t('fill_all')); return; }
    if (form.password !== form.confirmPassword) { setError(t('passwords_not_match')); return; }
    if (form.password.length < 6) { setError(t('password_too_short')); return; }
    setLoading(true);
    try {
      ...
      setError(Array.isArray(msg) ? msg.join(', ') : msg || t('register_failed'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-indigo-800 to-blue-900 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950 flex items-center justify-center px-4 py-8">
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-indigo-300 mt-2 text-sm">{t('register_subtitle')}</p>
        </div>

        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-6 sm:p-8">
          <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-6">{t('create_account')}</h2>
          {error && (
          ...
          <p className="text-center text-gray-600 dark:text-gray-400 text-sm mt-6">
            {t('have_account')}{' '}
            <Link to="/login" className="text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 font-medium">{t('go_login')}</Link>
          </p>
        </div>
      </div>
    </div>
