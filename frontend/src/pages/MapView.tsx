...
const MapView: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">🗺️ 同学分布地图</h1>
        <p className="text-gray-500 dark:text-gray-400 mt-1">共有同学分布在 {stats.length} 个城市</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-6 relative" style={{ minHeight: 480 }}>
            ...
            {/* Selected city popup */}
            {selected && (
              <div className="mt-4 bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-indigo-100 dark:border-gray-700">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-bold text-gray-900 dark:text-gray-100">📍 {selected.city} ({selected.count}人)</h3>
          ...
          <div className="space-y-3">
            <h2 className="font-bold text-gray-900 dark:text-gray-100 text-lg">城市排行</h2>
            {stats.map((city, i) => (
              <div
                key={city.city}
                onClick={() => setSelected(selected?.city === city.city ? null : city)}
                className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-colors ${selected?.city === city.city ? 'bg-indigo-100 border border-indigo-300' : 'bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 border border-gray-100 dark:border-gray-700'}`}
              >
                <span className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${i < 3 ? 'bg-indigo-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400'}`}>{i + 1}</span>
                <div className="flex-1">
                  <div className="flex justify-between items-center mb-1">
                    <span className="font-medium text-gray-900 dark:text-gray-100 text-sm">{city.city}</span>
                    <span className="text-indigo-600 dark:text-indigo-400 font-bold text-sm">{city.count}人</span>
                  </div>
            ...
            {stats.filter(c => !CITY_POSITIONS[c.city]).length > 0 && (
              <div className="mt-4">
                ...
                {stats.filter(c => !CITY_POSITIONS[c.city]).map(c => (
                  <div key={c.city} className="text-sm text-gray-600 dark:text-gray-400 py-1">{c.city}: {c.count}人</div>
                ))}
              </div>
            )}
          </div>
